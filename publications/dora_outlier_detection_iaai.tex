...
Many Earth science applications using satellite Earth observation (EO) data
 ...
 and Agriculture Organization (FAO). This dataset includes \todo{7XXX} samples
 with location (latitude/longitude) metadata covering \todo{a few countries}. 
 Most locations coincide with crop
 ...
 consisting of all samples in Kenya ($n=XX$) and manually annotated whether 
 ...
 Online~\cite{planetdata} ($n_{inlier}=XX$, $n_{outlier}=XX$). We used Google
 ...
 \todo{If we have time, I want to replace this with a different dataset and 
 experiment that is in progress that evaluates the downstream classifier
 performance before and after applying DORA.}
...
(University of Maryland) for providing the fall army worm ground observation
dataset.
