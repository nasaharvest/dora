...
the choice of which areas merit follow-up, deeper
investigation~\cite{kerner2020comparison,wagstaff:rover-novelty20}.
We collected \todo{XXX} images from the Mars Science Laboratory's
Navigation camera (Navcam) and employed the Rockster~\cite{burl:rockster16}
method used by onboard rover software to identify candidate rock
...
considered novel by the mission ($n_{novel} = 108$).
...
\subsubsection{Targets in Mars rover images.}
Kiri
% and organized them chronologically.
\subsubsection{Satellite time series for ground-truth observations}
Hannah: Describe the Earth crop field data set and applications
...
\todo{Funding/support: JPL CIF, SMD etc.}
We thank the NASA Center Innovation Fund for supporting this work.

\todo{Acknowledge people who contributed to 
