   ...
   cross-domain applicability of DORA, we conducted outlier detection
    ...
    datasets (Earth Science), 
     ...
     (astrophysics), and 3) novelty-guided onboard targeting of instruments
      for Mars rovers (planetary science). \todo{[We showed that ...]}
\end{abstract}
...
We used MNIST and F-MNIST to demonstrate the capabilities of DORA with a
traditional benchmark dataset. We used 60,000 images from 
F-MNIST as the training set and a test set of 1,000 images from MNIST (outliers)
 and 1,000 from F-MNIST (inliers). 
...
\paragraph{Evaluation in outlier detection.} Prior work has emphasized
