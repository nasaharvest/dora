...
class TestEarthVolcanoesPixels(TestCase):

    def setUp(self):

        self.outdir = '/tmp/dora/test/earth_volcanoes_pixels'
        if not os.path.exists(self.outdir):
            os.makedirs(self.outdir)

    def test_run(self):

        config_file = 'test/earth_volcanoes_pixels.config'

        # Run the experiment; also tests ability to parse config file
        start(config_file, self.outdir)

        # Check results for each algorithm
        for file_base in ['demud-k=1/selections-demud.csv',
                          ...
                          'pca-k=1/selections-pca.csv',
                          'rx/selections-rx.csv',
                          'random/selections-random.csv'
                          ]:
            correct_file = 'test/ref-results/earth_volcanoes_pixels/%s' % file_base
